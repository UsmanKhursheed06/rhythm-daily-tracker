...
export function useTracker(userId:string){
 ...
 async function sync(){if(busy.current)return;busy.current=true;try{
  ...
  const response=await apiFetch('sync',{cache:'no-store'});if(!response.ok)throw new Error(response.status===401?'Sign in to sync':'Sync unavailable · retrying');const data=await response.json() as {owner:string;records:Entry[]};
