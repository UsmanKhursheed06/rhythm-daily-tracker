export type Entry = { id: string; kind: 'task'|'habit'|'check'|'block'|'category'|'note'|'session'|'day'; title?: string; date?: string; category?: string; done?: boolean; start?: number; end?: number; days?: number[]; color?: string; minutes?: number; deleted?: boolean; version?: number };
